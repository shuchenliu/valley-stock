...
const ViewModel = () => {
    ...
    self.setShowCity = city => {
      ...
      ko.utils.arrayForEach(self.companyList(), eachCompany => {
        if (city == 'all' || city == eachCompany.city()) {
          eachCompany.marker.setVisible(true);
        } else {
          eachCompany.marker.setVisible(false);
        }
      });
    };
    ...
    self.shouldSideBarHide = ko.observable(true);

    self.toggleSidebar = () => {
      self.shouldSideBarHide(!self.shouldSideBarHide());
    }

    self.clickMapToHideSidebar = () => {
      if (!self.shouldSideBarHide()) {
        self.toggleSidebar();
      }
    }
